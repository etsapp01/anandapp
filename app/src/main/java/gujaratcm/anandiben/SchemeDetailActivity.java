...
public class SchemeDetailActivity extends BaseActivity {
    ...
    ImageView backdrop;
    SchemesInfo newsInfo;
    NewsDetailInfo detailInfo;
    RelativeLayout rlParent;
    ArrayList<SchemesInfo> m_array;
    int position = 0;
    Toolbar toolbar;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (b != null) {
            postid = b.getInt("id");
            newsInfo = SchemesInfo.getDataById(postid);
            detailInfo = NewsDetailInfo.getDetailByContainId(postid);
        }
        m_array = SchemesInfo.getAll();
        if (m_array != null) {
            if (newsInfo != null) {
                for (SchemesInfo news : m_array) {
                    if (news.schemeid == postid) {
                        break;
                    }
                    position++;
                }
        ...
        facebook_share.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (newsInfo != null)
                    FacebookSharing(SchemeDetailActivity.this, newsInfo.link);
            }
        });
        ...
        google_share.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                GoogleSharing(SchemeDetailActivity.this);
            }
        });
