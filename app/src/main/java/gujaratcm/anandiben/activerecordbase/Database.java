...
public class Database {
	...
	public void open() {
		...
		Logg.d(ARConst.TAG, "(%t) %s.open(): new db obj %s", CNAME,
				_database.toString());
	}
	...
	public String[] getTables() throws ActiveRecordException {
		if (null == _database || !_database.isOpen()) {
			Logg.e(ARConst.TAG,
					"(%t) %s.getTables(): ERROR - db object is null or closed",
					CNAME);
			throw new ActiveRecordException(ErrMsg.ERR_DB_IS_NOT_OPEN);
		}
	...
	public int getVersion() throws ActiveRecordException {
		if (null == _database || !_database.isOpen()) {
			Logg.e(ARConst.TAG,
					"(%t) %s.getVersion(): ERROR - db object is null or closed",
					CNAME);
			throw new ActiveRecordException(ErrMsg.ERR_DB_IS_NOT_OPEN);
		}
	...
	public void setVersion(int version) throws ActiveRecordException {
		if (null == _database || !_database.isOpen()) {
			Logg.e(ARConst.TAG,
					"(%t) %s.setVersion(): ERROR - db object is null or closed",
					CNAME);
			throw new ActiveRecordException(ErrMsg.ERR_DB_IS_NOT_OPEN);
		}
